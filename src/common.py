...
def get_tensor_from_camera(RT, Tquad=False):
    ...
    参数:
        RT: 相机的变换矩阵，包含旋转和平移。
        Tquad (bool): 如果为True, 则平移向量T和四元数quad的顺序为[T, quad]，否则为[quad, T]。

    返回:
        tensor: 包含四元数和平移向量的张量
    """
    gpu_id = -1
    if type(RT) == torch.Tensor:
        # 如果RT是一个张量，首先检查其是否在GPU上，如果是，则将其转移到CPU，并记录GPU的ID
        if RT.get_device() != -1:
            RT = RT.detach().cpu()
            gpu_id = RT.get_device()
        RT = RT.numpy()
    # 使用mathutils库的Matrix类处理旋转矩阵R和平移向量T
    from mathutils import Matrix
    R, T = RT[:3, :3], RT[:3, 3] # 分离旋转矩阵R和平移向量T
    rot = Matrix(R) # 将旋转矩阵R转换为Matrix对象
    quad = rot.to_quaternion() # 将旋转矩阵转换为四元数
    if Tquad:
        # 根据Tquad参数的值决定平移向量T和四元数quad的顺序
        tensor = np.concatenate([T, quad], 0)
    else:
        tensor = np.concatenate([quad, T], 0)
    tensor = torch.from_numpy(tensor).float() # 将numpy数组转换为torch张量
    if gpu_id != -1:
        # 如果原来的RT张量在GPU上，将结果张量也转移到相同的GPU
        tensor = tensor.to(gpu_id)
    return tensor # 返回包含四元数和平移向量的张量
